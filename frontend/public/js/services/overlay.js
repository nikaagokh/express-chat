...
import { PostReactions } from "../general/post-reactions.js";
import { PostComments } from "../general/post-comments.js";
import { Group } from "../general/group.js";
...
class OverlayService {
    ...
    openAddGroup() {
        if (!this.addGroupOverlay) {
            ...
            this.addGroupOverlay.close$.addEventListener('closed', () => {
                this.addGroupOverlay = null;
                this.enableScroll();
    ...
    openPostComments(post_id) {
        if (!this.userComments) {
            const { globalWrapper, dialogBackdrop, dialogComponent } = this.createDialog();
            const userComments = new PostComments(post_id);
            dialogComponent.appendChild(userComments.element);
            globalWrapper.appendChild(dialogComponent)
            this.disableScroll();
            this.overlayContainer.appendChild(dialogBackdrop);
            this.overlayContainer.appendChild(globalWrapper);
            userComments.close$.addEventListener('closed', () => {
                this.userComments = null;
                this.enableScroll();
                globalWrapper.remove();
                dialogBackdrop.remove();
            ...
            this.userComments.detach();
            this.userComments = null;
        }
    }

    openPostReactions(type, post_id) {
        if (!this.userReactions) {
            const { globalWrapper, dialogBackdrop, dialogComponent } = this.createDialog();
            const userReactions = new PostReactions(type, post_id);
    ...
    openGroup(conversationId, chatName) {
        console.log(this.chatOverlay)
        if (!this.chatOverlay) {
            const { globalWrapper, overlayBackDrop, chatComponent } = this.createChatOverlay();
            this.chatOverlay = new Group(conversationId, chatName);
